...
USAGE:
  python path/to/alma-batch-upload-script-lp.py --auto
    (this will find the latest batch-upload-alma-lp-<timestamp>.txt file from workflow output)
  python path/to/alma-batch-upload-script-lp.py path/to/other/input/file.txt
  python path/to/alma-batch-upload-script-lp.py --auto --delimiter ','
"""
...
def convert_oclc_json_to_marcxml(oclc_json_data):
    ...
    # 007 - Physical Description Fixed Field (for LP)
    field007 = ET.SubElement(root, 'controlfield')
    field007.set('tag', '007')
    field007.text = 'sd fungnnmmned'  # Standard for analog LP (not digital)
...
def convert_brief_record_to_marcxml(brief_record):
    ...
    field007 = ET.SubElement(root, 'controlfield')
    field007.set('tag', '007')
    field007.text = 'sd fungnnmmned'  # Standard for analog LP
    ...
    # 300 - Physical description (RECOMMENDED for LPs)
    field300 = ET.SubElement(root, 'datafield')
    field300.set('tag', '300')
    field300.set('ind1', ' ')
    field300.set('ind2', ' ')
    subfield_a = ET.SubElement(field300, 'subfield')
    subfield_a.set('code', 'a')
    subfield_a.text = '1 audio disc :'  # Can be multiple for LP sets
...
def create_holding(mms_id, library_code, location_code):
    """Create a minimal holding record (neutral—no donor/541)."""
    url_holdings = f"{ALMA_BASE}/bibs/{mms_id}/holdings"
...
if __name__ == "__main__":
    ...
    # Positional argument for input file - for example, python script.py input.txt
    parser.add_argument(
        'input_file',
        nargs='?',
        help='Path to pipe-delimited input file (oclcNumber|barcode|title)'
    )
    # Optional flag to auto-detect latest batch upload file if you have run full workflow
    parser.add_argument(
        '--auto',
        action='store_true',
        help='Automatically find latest batch upload file'
    )
    # argument to change delimiter, if you have commas and not pipes, for example, python --auto --delimiter ','
    parser.add_argument(
        '--delimiter', 
        default='|',
