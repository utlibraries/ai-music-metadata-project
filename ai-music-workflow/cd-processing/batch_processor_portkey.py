"""
Portkey Batch Processing Module for AI Music Metadata Project (CD)

This module is a Portkey-native version of batch_processor.py. All API calls
(file uploads, batch submission, status checks, result downloads) are routed
through the Portkey AI gateway using the Portkey Python SDK, which is
OpenAI-API-compatible.

Use this module directly when you want explicit Portkey routing for batch jobs.
Alternatively, set PORTKEY_CONFIG["enabled"] = True in cd_workflow_config.py and
the standard batch_processor.py will use Portkey automatically.

Requires environment variables:
    PORTKEY_API_KEY       — your Portkey gateway API key
    PORTKEY_VIRTUAL_KEY   — the virtual key slug for your OpenAI provider connection
"""
...
from cd_workflow_config import get_model_config, get_token_limit_param, get_temperature_param, PORTKEY_CONFIG
...
class BatchProcessor:
    """
    Handles Batch API operations routed through the Portkey AI gateway.
    ...
    def __init__(self, api_key: Optional[str] = None, default_step: str = "step1", persistence_dir: Optional[str] = None):
        """
        Initialize batch processor with Portkey client.

        Args:
            api_key: Unused — Portkey credentials are read from environment variables
                     PORTKEY_API_KEY and PORTKEY_VIRTUAL_KEY.
        ...
        if not portkey_api_key or not portkey_virtual_key:
            raise EnvironmentError(
                f"Portkey batch processor requires {PORTKEY_CONFIG['api_key_env']} and "
                f"{PORTKEY_CONFIG['virtual_key_env']} environment variables to be set."
            )
    ...
    def resume_batch(self, batch_id: str, max_wait_hours: int = 24, check_interval_minutes: int = 5) -> Optional[List[Dict[str, Any]]]:
        ...
        if batch_id not in self.batch_jobs:
            ...
            if "error" in status_info:
                print(f"Batch {batch_id} not found in state or API")
                return None
    ...
    def submit_batch(self, batch_requests: List[Dict[str, Any]],
                    description: str = "") -> str:
        """
        Submit a batch job via Portkey and return the batch ID.
